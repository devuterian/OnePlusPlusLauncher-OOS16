...
| Launcher state transition hooks (`onStateSetStart`, `onStateSetEnd`) | OEM launcher updates frequently move all-apps transition timing | Does all-apps entry still trigger exactly once on swipe-up without redirect false positives? |
| Search manager lookup (`getSearchUiManager`, fallback fields/methods) | Keyboard focus fails silently when manager accessor names change | Do manager accessors resolve on current launcher and still expose focus + keyboard paths? |
| Swipe-down redirect entry (`WorkspacePullDownDetectController.showSearchBar`) | Method signature drift can disable redirect path completely | Is method signature unchanged, and is redirect/focus behavior still consistent under animation? |
